    ...
    onValue(bannersRef, (snapshot) => {
      ...
      if (bannersData) {
        ...
        const bannersFiltrados = bannerList.filter((banner) => {
          return banner.provincias.includes(user?.provincia) && banner.sectores.includes(user?.sector);
        });
      ...
      {banners.length > 0 ? (
        <Slider {...settings}>
          {banners.map((banner, index) => (
            ...
            >
              {banner.link ? (
                <a href={banner.link} target="_blank" rel="noopener noreferrer" style={{ width: '100%', height: '100%' }} onClick={() => registrarClick(banner.id)}>
                  <img 
                    src={banner.imageUrl} 
                    alt={`Banner ${index + 1}`} 
