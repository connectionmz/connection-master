...
const FeedDesk = ({ user }) => {
  ...
  const formatTimestamp = (timestamp) => {
    ...
    return isToday ? `As ${formattedTime}` : date.toLocaleDateString('pt-BR') + ' ' + formattedTime;
  };
        ...
        {posts.map((post) => (
          <Grid item xs={12} sm={6} md={4} lg={3} key={post.id}>
            ...
            >
              ...
              />
              <CardContent>
                <Typography variant="body2" color="text.secondary" dangerouslySetInnerHTML={{ __html: post.description || 'Sem descrição' }} />
              </CardContent>
              ...
              <Box sx={{ 
                ...
                {post.companyId === user.id && (
                  <IconButton size="small" sx={{ color: '#ff4d4d' }} onClick={(e) => { 
                    e.stopPropagation(); 
